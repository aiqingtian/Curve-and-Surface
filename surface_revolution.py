# Generate a surface by revolving curve along a line connects the first control point to the last control point of Bezier curve using Blender
import numpy as np
import math
import bpy
import bmesh
...
last_point = (cur_x[pts_len- 1], cur_y[pts_len- 1], cur_z[pts_len- 1])
axiss = tuple([(first_point[i] - last_point[i])/2 for i in range(3)])
ax_cur = [pt-first_point for pt in cur]
verts, faces = makeMesh('curve', steps=128, pts=ax_cur, axis=axiss)
createMeshFromData('curve', origin=first_point, verts=verts, faces=faces)